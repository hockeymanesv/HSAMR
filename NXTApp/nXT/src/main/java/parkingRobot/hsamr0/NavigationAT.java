...
public class NavigationAT implements INavigation {
	 ...
	 */
	static final double RIGHT_WHEEL_RADIUS = 0.027315; // only rough guess, to be
													// measured exactly and
													// maybe refined by
													// experiments
	/**
	...
	static final double FRONT_SIDE_SENSOR_OFFAXIS = 0.07;
	...
	static final double ENCODER_RELATIVE_ANGLE_ERROR = 0.05;
	static final double ENCODER_RELATIVE_POSE_ERROR = 0.025;
	...
	private void calculateLocation() {
		...
		errorAngle = w * deltaT * ENCODER_RELATIVE_ANGLE_ERROR;
		errorX = Math.abs(this.pose.getX() - xResult) * ENCODER_RELATIVE_POSE_ERROR;
		errorY = Math.abs(this.pose.getY() - yResult) * ENCODER_RELATIVE_POSE_ERROR;
		 ...
		 angleMouse = this.mouseOdoMeasurement.getUSum()/MOUSE_SENSOR_OFFSET; //Einheiten �berlegen
		 ...
		 if(onLine){
			...
			if(angleOverwrite){
				errorX = errorX +errorAngle * 3;
				errorY = errorY +errorAngle * 3;
				errorAngle = 0.17;
			}
...
//				Sound.beepSequenceUp();
				angleResult = angleResult - (aMax + aMin)/2+Math.PI/4;
				angleOverwrite = false;
				aMax = 0;
				aMin = 0;
...
//				Sound.beepSequenceUp();
				angleResult = angleResult - (aMax + aMin)/2+Math.PI*3/4;
				angleOverwrite = false;
				aMax = 0;
				aMin = 0;
...
//				Sound.beepSequenceUp();
			...
			if(Math.abs(yResult-0.15)< 0.05 && Math.abs(angleResult - 3/2 * Math.PI) < 0.15){
				xResult = -0.2 + frontEffective;
				yResult = -frontSideEffective+0.8;
				errorX = 0.03;
				errorY = 0.03;
...
//				 angleLimited = angleResult -Math.PI*2;
//			 }
//			 
//			 if (angleResult < -Math.PI) { 
//				 angleLimited = angleResult + Math.PI*2;
...
//			aTriangSS = Math.atan((frontSideEffective - backSideEffective) / distanceSideSensor);
//			xRobo = backSideEffective + Math.tan(angleResult) * BACK_SIDE_SENSOR_OFFAXIS;
//			yRoboFront = frontEffective + Math.tan(Math.PI/2 + angleResult) * FRONT_SENSOR_OFFAXIS;
//			yRoboBack = backEffective + Math.tan(angleResult) * BACK_SENSOR_OFFAXIS;
//			xRoboTrue = xRobo * Math.cos(angleResult);
//			yRoboTrueFront = yRoboFront * Math.cos(angleResult);
//			yRoboTrueBack = yRoboBack * Math.cos(angleResult);
...
//			if(this.frontSideSensorDistance < 150 && this.backSideSensorDistance < 150){
//				if(true|| Math.abs(angleResult) < Math.PI/12 && yResult < 0.05 && xResult > 1.7){
//					aTriang = aTriangSS;
//					xTriang = frontEffective;
//					yTriang = 0.798-xRoboTrue;
//					triangOverwirte = true;
//				}
//				if(false && Math.abs(angleResult) > 11*Math.PI/12 && yResult < 0.55 && Math.abs(xResult-0.165) < 0.10){
//					aTriang = aTriangSS + Math.PI;
//					xTriang = 1.898-yRoboTrueFront;
//					yTriang = -0.098+xRoboTrue;
//					triangOverwirte = true;
//				}
//			}
//			
